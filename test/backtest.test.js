...
test('backtest: Turtle 4h — signal on close, fill at next open, stop fixed at entry, fees + slippage', () => {
  const c = cfg();
  ...
  const r = backtestStrategy({ strategy: 'TURTLE', config: c, data, start, end, capital: 1_000_000, symbols: ['BTCUSDT'] });
...
test('backtest: intrabar emergency stop and gap-through fills', () => {
  const c = cfg();
  ...
  const r = backtestStrategy({ strategy: 'TSMOM', config: c, data: { BTCUSDT: bars }, start: 79 * DAY, end: 82 * DAY, capital: 1_000_000, symbols: ['BTCUSDT'] });
  ...
  const r2 = backtestStrategy({ strategy: 'TSMOM', config: c, data: { BTCUSDT: bars2 }, start: 79 * DAY, end: 82 * DAY, capital: 1_000_000, symbols: ['BTCUSDT'] });
  const t2 = r2.trades.find((x) => x.reason === 'FIXED_STOP');
  assert.ok(Math.abs(t2.exitPrice - 120 * (1 - 0.0005)) < 1e-6);
});

test('backtest: compound vs fixed sizing, equity accounting consistent', () => {
  const c = cfg();
  ...
  const a = backtestStrategy({ ...args, compound: true });
  const b = backtestStrategy({ ...args, compound: false });
...
test('backtest: re-arm after stop for ADX-type strategies, three symbols share one account', () => {
  const c = cfg();
  const closes = Array.from({ length: 120 }, (_, i) => 100 + i);
  const data = { BTCUSDT: mk(closes, DAY), ETHUSDT: mk(closes, DAY), XRPUSDT: mk(closes, DAY) };
  const r = backtestStrategy({ strategy: 'TSMOM', config: c, data, start: 50 * DAY, end: 120 * DAY, capital: 1_000_000 });
...
test('backtest: QQQ with limited history reports warm-up instead of fake trades', () => {
  const c = cfg();
  const bars = mk(Array(60).fill(600), DAY);
  const r = backtestStrategy({ strategy: 'QQQ_EMA_TREND', config: c, data: { QQQUSDT: bars }, start: 0, end: 60 * DAY, capital: 1_000_000 });
