...
export class Controller {
  ...
  keys() { return STRATEGIES.flatMap((st) => SIDES.map((side) => ({ strategy: st, side, key: sideKey(st, side) }))); }
  ...
  metricsFor(strategy, side, now) {
    ...
    const trades = this.shadow.state.trades.filter((t) => t.strategy === strategy && t.side === side);
    return computeMetrics({ series: this.shadow.state.series[k] || [], trades, nSlots: this.symbols.length, windows: this.cfg.windows, lowTradeCount: this.cfg.lowTradeCountForExtended, now });
  }
  ...
  correlations() {
    ...
    for (let i = 0; i < STRATEGIES.length; i++) for (let j = i + 1; j < STRATEGIES.length; j++) {
      ...
      const corr = returnCorrelation(this.strategySeries(a), this.strategySeries(b), this.symbols.length, this.cfg.windows.main);
      out.push({ a, b, corr, high: corr != null && corr > this.cfg.guards.correlationThreshold });
    }
    return out;
  }
  ...
  exposure() {
    ...
    return { equity, long: acct.longExp, short: acct.shortExp, net: acct.netExp, gross: acct.longExp + acct.shortExp, coins };
  }
  ...
  evaluate(trigger = 'manual', now = Date.now()) {
    try {
      ...
      const reg = detectRegime(this.md.s.BTCUSDT?.daily || [], this.cfg.regime);
      ...
      for (const { strategy, side, key } of this.keys()) {
        ...
        const d = decide({
          metrics, prevStatus: prev, side, regime: reg.regime, sideActive: this.sideActive(strategy, side),
      ...
      for (const { strategy, side, key } of this.keys()) {
        ...
        if (mode === 'LIVE_APPROVAL' && d.status !== liveCur) {
          pendingId = `${key}:${now}`;
          this.state.pending.push({ id: pendingId, key, strategy, side, from: liveCur, to: d.status, fromMult: table[liveCur], toMult: mult, reason, createdAt: now, regime: reg.regime });
        }
        this.cstore.appendHistory(historyRecord({
          now, mode, strategy, side, prev: d.prev, next: d.status, table, m, regime: reg.regime, reason, trigger,
      ...
      this.cstore.saveNow();
      this.log.info(`Controller evaluation (${trigger}, ${mode}) regime ${reg.regime}: ${summary.length ? summary.join(', ') : 'no status change'}${this.state.pending.length ? ` — ${this.state.pending.length} LIVE recommendation(s) pending approval` : ''}`, 'CONTROLLER_EVAL');
      return { ok: true };
    } catch (e) {
      this.enterFailSafe(e);
  ...
  compact() {
    ...
    const rows = this.keys().map(({ strategy, side, key }) => {
      ...
      return { strategy, side, key, active: this.sideActive(strategy, side), recommended: s?.status || 'NORMAL', recommendedMult: table[s?.status || 'NORMAL'], applied: sizing.status, appliedMult: sizing.multiplier };
    });
    return {
      mode, failSafe: this.state.failSafe, lastEvaluation: this.state.lastEvaluation, nextEvaluation: this.nextEvaluation(),
      regime: this.state.regime?.regime || null, pending: this.state.pending.length, rows,
  ...
  detail() {
    ...
    const strategies = this.keys().map(({ strategy, side, key }) => {
      ...
      return {
        strategy, side, key, active: this.sideActive(strategy, side), enabled: this.store.config.strategies[strategy].enabled,
    ...
    return {
      ...this.compact(), tradingMode, table, config: this.cfg, regimeDetail: this.state.regime,
...
function historyRecord({ now, mode, strategy, side, prev, next, table, m, regime, reason, trigger, approved, applied, appliedTo, prevApplied, pendingId }) {
  return {
